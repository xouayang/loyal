...
const CarryOutDocument = require('../models/carry.out.document.model');

exports.create = (req, res) => {
    const { MEMBER_ID } = req.payload;
    CarryOutDocument.create({ ...req.body, memberId: MEMBER_ID }).then((data) => {
        if (data) {
...
exports.update = (req, res) => {
    ...
    const where = { id: id };
    CarryOutDocument.update({ ...req.body }, { where: where }).then((data) => {
        if (data[0] == 1) {
            CarryOutDocument.findOne({
                where: where
            }).then((data) => {
                if (data) {
                    return res.status(200).json({ result: data });
...
exports.findAllByMember = async (req, res) => {
    ...
    try {
        const count = await sequelize.query(`SELECT COUNT(id) AS count FROM carry_out_documents
        WHERE memberId='${MEMBER_ID}' AND status=${status}`,
            { type: sequelize.QueryTypes.SELECT });
        const datas = await sequelize.query(`SELECT [id]
        ,[memberId]
        ,[name]
